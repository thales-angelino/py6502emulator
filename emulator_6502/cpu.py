...
class CPU(object):
    ...
    def execute(self, n_opcodes=None):
        while self.program_counter < 0xffff:
            opcode = self.fetch_byte()
            self.run_opcode(opcode)

            if n_opcodes is not None:
                print "n opcode: %d" % n_opcodes
                n_opcodes -= 1
    ...
    def print_cpu_stats(self):
        stats = (self.cycles,
                 self.processor_status['carry'],
                 self.processor_status['zero'],
                 self.processor_status['interrupt_disable'],
                 self.processor_status['decimal_mode'],
                 self.processor_status['break_command'],
                 self.processor_status['overflow'],
                 self.processor_status['negative'],
                 hex(self.a),
                 hex(self.x),
                 hex(self.y),
                 hex(self.program_counter),
                 hex(self.stack_pointer),)
        print('Cycles\t|C\t|Z\t|I\t|D\t|B\t|O\t|N\t|A\t|X\t|Y\t|PC\t|SP')
        print('%d\t|%d\t|%d\t|%d\t|%d\t|%d\t|%d\t|%d\t|%s\t|%s\t|%s\t|%s\t|%s' % stats)