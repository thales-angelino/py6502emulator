...
from emulator_6502.instructions import tsx
...
class TestTSX(unittest.TestCase):
    ...
    def test_tsx(self):
        expected_value = 0xff
        expected_cycles = 2
        self.memory.memory[emulator.START_ADDRESS] = tsx.TSX_IMPLIED_OPCODE
        self.cpu.execute(1)
        self.assertEqual(self.cpu.x, expected_value, "Register X should contain: %s" % hex(expected_value))
